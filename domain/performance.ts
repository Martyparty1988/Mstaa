import { WorkLog, Worker, ProjectPerformance, Forecast, WorkType, Project, PerformanceSnapshot, WorkerPerformance, ProjectSettings } from './types';
import { calculateLogStrings, stringsToKwp } from './rules';

/**
 * Creates a unified PerformanceSnapshot from a set of logs.
 * Now accepts optional ProjectSettings to customize calculations.
 */
export const createPerformanceSnapshot = (logs: WorkLog[], settings?: ProjectSettings): PerformanceSnapshot => {
  let hours = 0;
  let strings = 0;
  let tables = 0;
  ...
  logs.forEach(log => {
    ...
    if (log.type === WorkType.TABLE) {
       strings += calculateLogStrings(log, settings);
  ...
  return {
    ...
    stringsPerHour,
    tablesPerDay,
    kwp: stringsToKwp(strings, settings)
  };
};
 ...
 */
export const calculatePerformance = (logs: WorkLog[], workers: Worker[], timeRange: 'DAY' | 'WEEK' | 'ALL', project?: Project): ProjectPerformance => {
  const now = new Date();
  ...
  const globalSnapshot = createPerformanceSnapshot(filteredLogs, settings);
  ...
  const workerPerformances: WorkerPerformance[] = Object.entries(workerLogs).map(([wId, wLogs]) => {
    const wSnapshot = createPerformanceSnapshot(wLogs, settings);
...
export const forecastCompletion = (project: Project, logs: WorkLog[]): Forecast => {
  ...
  const recentSnapshot = createPerformanceSnapshot(recentLogs, project.settings);
